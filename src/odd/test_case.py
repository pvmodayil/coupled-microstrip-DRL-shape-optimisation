...
def get_arrangement(row: pd.Series, model_mode: str) -> CoupledStripArrangement:
    CSA: CoupledStripArrangement = CoupledStripArrangement(
        V0=row['V0'], # Potential of the sytem, used to scale the system which is defaulted at V0=1.0
        hw_arra=row['a'], # half width of the arrangement, parameter a
        ht_arra=row['c'], # height of the arrangement, parameter b
        ht_subs=row['h'], # height of the substrate, parameter h
        space_bw_strps=row['s'], # gap between the two microstrips, parameter s
        width_micrstr=row['w'], # width of the microstrip, parameter w
        ht_micrstr=0, # height of the microstripm, parameter t
        er1=1.0, # dielectric constatnt for medium 1
        er2=row['er2'], # dielctric constant for medium 2
...
def main(model_pathD: str, model_pathL: str, test_case_file: str, model_mode: str) -> None:
    ...
    result_file: str = os.path.splitext(os.path.basename(test_case_file))[0] + "_result.xlsx"
    df_test.to_excel(os.path.join(os.getcwd(),"test",model_mode,result_file))


if __name__ == "__main__":
    
    model_pathD = os.path.join("training","ODD","TC3","caseD","models","SAC_CSA_ODD.zip")
    model_pathL = os.path.join("training","ODD","TC3","caseL","models","SAC_CSA_ODD.zip")
    test_case_file: str = os.path.join(os.getcwd(), "test", "TestCase1-3_Odd.csv")
    main(model_pathD=model_pathD,model_pathL=model_pathL, test_case_file=test_case_file, model_mode = "ODD")